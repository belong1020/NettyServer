...
public class BitMap {
	...
	private int length;
	private final static int[] bitValue = { 0x80000000, 0x40000000, 0x20000000, 0x10000000, 0x08000000, 0x04000000,
	...
	/*
	 * public byte[] addBits(String regix){ String[] vals = regix.split(",");
	 * for(String val:vals){ //本处-1的原因是，BitMap在1~64之间，而实际存储为0~63
	 * addBit(Integer.valueOf(val)-1); } return getBitMap(); }
	 */
	...
	/**
	 * 根据返回的bit值 --- 形参byte[1,2]
	 * 
	 * @param bits
	 * @return
	 */
	public static BitMap addBitmap(byte[] bits) {
		BitMap bitMap = new BitMap(bits.length * 8);
		int pointer = 2;

		for (int i = 1; i < bits.length; i++) {
			if (bits[i] == 49)
				bitMap.addBit(pointer - 1);
			pointer++;
		}
		return bitMap;
	}
