...
/**
 * Old Netty Server bootstrap class
 *
 * @author 
 */
@Deprecated
public class NettyServerOld {
    private final int port;

    public NettyServerOld(int port) {
        this.port = port;
    }
...
//    	ServerConfig.getInstance().getServerPort()
    	new NettyServerOld(65535).start();
            ...
            b.group(group)
                ...
                .childHandler(new ChannelInitializer<SocketChannel>() {
                    ...
                    public void initChannel(SocketChannel ch) throws Exception {
                        ...
                        //这将在后面的章节中讲到。
                        ch.pipeline().addLast("serverHandler", new UnpackHandler());
                        ch.pipeline().addLast("authHandler", new AuthHandler());
                        ch.pipeline().addLast("packHandler", new PackHandler());
                    }
                });
            //(6) 异步地绑定服务器；调用 sync()方法阻塞等待直到绑定完成
            ChannelFuture f = b.bind().sync();
            System.out.println(NettyServerOld.class.getName() +
